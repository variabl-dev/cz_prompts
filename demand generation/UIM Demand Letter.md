...
This is an **Underinsured Motorist (UIM)** settlement demand — secondary to a previously resolved third-party bodily injury claim against the at-fault driver. Key implications:

- The letter is addressed to **the client's own automobile insurance carrier** (the UIM carrier)
...
- The introduction MUST reference the previously concluded third-party settlement (at-fault driver's bodily injury policy limits, prior insurance carrier name, and prior claim number). This context is in the **DISB** folder.
...
**California** (default if no state identified): retain California citations (Cal. Veh. Code §, Evidence Code § 1152, P.S.T.).
...
- Settlement inadmissibility → **Federal Rule of Evidence 408**: "this is a confidential settlement demand as contemplated by Federal Rule of Evidence 408 and is inadmissible at arbitration in the matters of liability and damages."
...
Example – UIM Introduction (California):
...
Generate a complete UIM Demand Letter using ONLY the provided data. Do NOT fabricate facts, infer missing details, or include placeholders. If information is missing, omit it naturally.
...
2. Introduction (prior third-party settlement context required)
3. Facts
4. Liability
...
- **`your_insured`**: For UIM, same as our_client (client is the insured under their own policy)
- **`claim_no`**: The UIM carrier's claim number
...
Describe the accident clearly and chronologically. Use only verified facts. Focus on the at-fault third-party driver's conduct.
...
- State the specific negligent conduct
- Reference applicable vehicle/traffic code sections (bold) per the filing state
- Reference police reports, citations, or liability acceptance if available
...
**Do NOT state the UIM policy limits dollar amount.** Refer only to "the available underinsured motorist policy limit."
...
1. **Settlement offer:** Offer to resolve for the available UIM policy limit. State the deadline. State that if not resolved by that time, the client will demand arbitration.
...
- Do NOT mention internal systems (LITIFY, folders, OCR, DISB) in the letter text
...
| `your_insured` | string | For UIM: same as `our_client` |
| `claim_no` | string | UIM carrier's claim number |
...
| `loss_of_income` | string | Empty string `""` (not applicable for UIM) |
