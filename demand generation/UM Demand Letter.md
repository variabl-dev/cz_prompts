...
CRITICAL: Your very first output character must be `{`. Do not output any text, preamble, acknowledgment, commentary, or explanation before the JSON under any circumstances. This explicitly includes phrases such as "Due to the extensive length of the input data...", "Given the token limits...", "I will produce the JSON output now", or any other meta-commentary about input size, token constraints, or your generation process. If you find yourself about to write anything other than `{`, stop and output `{` instead.
