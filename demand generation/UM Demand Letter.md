...
CRITICAL: Your very first output character must be `{`. Do not output any text, preamble, acknowledgment, commentary, or explanation before the JSON under any circumstances. Never reference or acknowledge input length, token limits, context length, output length, your ability to complete the task, or whether the response fits in a single output — regardless of how the thought is phrased. Do not "announce" that you are about to produce the JSON. Do not express any caveats or uncertainty before beginning. If you find yourself about to write anything other than `{`, stop and output `{` instead.
...
This is an **Uninsured Motorist (UM)** settlement demand. The at-fault driver had no available bodily injury coverage — commonly because they were uninsured, fled the scene (hit-and-run), or are an unidentified motorist. Key implications:

- The letter is addressed to **the client's own automobile insurance carrier** (the UM carrier), not the at-fault driver's carrier
- In the Re: block, "Our Client" and "Your Insured" will typically be the **same person** — the client is the insured under their own UM policy
- The demand is for the **UM policy limits** of the client's own policy
- The at-fault driver may be an "unidentified motorist" or hit-and-run driver — describe them accordingly in Facts and Liability
...
Generate a complete UM Demand Letter using ONLY the provided data.
...
6. Damages
   ...
   - Pain & Suffering
7. Conclusion (short UM settlement demand language)
...
2. **Date** — full date (e.g., "February 18, 2026"), **centered**: `<p style="font-family: 'Times New Roman', serif; margin-bottom: 12pt; text-align: center;">[Date]</p>`
3. **Sent Via Email line** — bold and italic: `Sent Via Email: [email address]`
4. **Adjuster block** — adjuster name, insurance company name (the UM carrier), address (each on its own line)
5. **Re: block** — use a full-width table with empty spacer columns on both sides to center the content on the page. Use HTML `width` attributes on each `<td>` (NOT CSS width, NOT `<colgroup>`). The label column must be right-aligned (`text-align: right`) so the colons line up vertically. For a UM demand, "Our Client" and "Your Insured" will typically be the same person — the client is the insured under their own UM policy. The "Claim No." is the UM carrier's claim number:
   ```
...
6. **Title** — centered, bold, underlined: "UNINSURED MOTORIST SETTLEMENT DEMAND"
...
State purpose of letter — a formal request to settle the claim for the client's uninsured motorist policy limits.
This is NOT a numbered section — it appears between the salutation and Section 1.
Keep it brief — typically one short paragraph is sufficient.
...
### Conclusion (UM Settlement Demand — Short Form)

**CRITICAL — The UM conclusion is intentionally short and light.** Do NOT include:
...
Do NOT import third-party policy limits demand language (conditions precedent, declarations under penalty of perjury, etc.) — this is a UM demand with a short conclusion
...
{"header":"Claim No.: 12345\nFebruary 18, 2026","body":"<div><p style=\"font-family: 'Times New Roman', serif; margin-bottom: 12pt;\">Content here...</p></div>","footer":"CARPENTER & ZUCKERMAN\n8827 W. Olympic Blvd., Beverly Hills, CA 90211 T 310-273-1230 F 310-858-1063"}

### `header` key

Plain text content for the Google Docs repeating page header. Format as two lines separated by \n: first line is "Claim No.: [claim number]", second line is the full date of the letter. The claim number here is the **UM carrier's claim number**. Do NOT include HTML tags in this value — plain text only.
